...
class RedBlackTreeMap(TreeMap):
        ...
        if self.is_root(p):
            ...
            if self._is_red(parent):                    # double red problem
                ...
                if not self._is_red(uncle):             # Case 1: misshapen 4-node
                    ...
                    self._set_black(self.left(grand))   # its children become red
    ...
    def _rebalance_delete(self, p):
        if len(self) == 1:
            ...
            if n == 1:                                  # deficit exists unless child is a red leaf
                ...
                if not self._is_red_leaf(c):            # removed black node with red child
    ...
    def _fix_deficit(self, z, y):
        ...
        else:                                           # Case 3: y is red; rotate misalogned 3-node and repeat
            self._rotate(y)
            self._set_black(y)
            self._set_red(z)
            if z == self.right(y):
